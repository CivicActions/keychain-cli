...
- [ ] T008 [P] Implement `src/keychain_cli/errors.py`: base `KeychainCliError(message, next_step, exit_code)`; subclasses `UsageError`(2), `NotFoundError`(3), `RefusedError`(4), `KeychainError`(5, carries `os_status: int` and `os_message: str`), `UnsupportedPlatformError`(6), `InvalidInputError`(7), `CommandNotFoundError`(8), `ClipboardError`(9); non-error result constants `EXIT_CHECK_MISSING = 10` and `EXIT_INTERRUPTED = 130`; `format_error()` producing `keychain-cli: <what failed>. <what to do next>`; no constructor accepts bytes, a `SecretValue`, or an integer length of a value
...
- [ ] T023 [P] Implement `tests/integration/conftest.py`: skip the whole directory unless `sys.platform == "darwin"`; fixture `temp_keychain` using `TemporaryKeychain` under `tmp_path`; fixture `real_store` returning `SecurityFrameworkStore(keychain=temp_keychain)`; fixture `sandbox` that creates fresh `home/`, `tmp/`, and `work/` directories under `tmp_path` and returns an environment dict with `HOME`, `TMPDIR`, and `KEYCHAIN_CLI_TEST_KEYCHAIN=<temp keychain path>`; fixture `cli_env(argv, stdin=None)` that runs the installed `keychain-cli` via `subprocess` inside `sandbox` (cwd `work/`) and returns `(exit_code, stdout, stderr)`; helper `assert_sandbox_clean(sandbox, leak)` asserting no file was created under `home/`, `tmp/`, or `work/` beyond files the test itself wrote, and that `leak` appears in no file beneath them (FR-036, SC-009). The store factory honors `KEYCHAIN_CLI_TEST_KEYCHAIN` **only** when set and `cli.py` prints `using test keychain <path>` on stderr whenever it is (contracts/cli.md "Environment variables")
...
- [ ] T025 [P] Unit tests `tests/unit/test_secret_redaction.py`: `repr`, `str`, `f"{v}"` (raises), `"{}".format(v)` (raises), `json.dumps` (raises), `SecretValue` inside every `errors.py` exception is rejected by type; `format_error()` output for each exception class contains no `LEAK`; **FR-038 length check**: for a 1,234-byte value, every error path (`StoreError` from a fake store rejecting the add, empty-value refusal, stdin refusal) produces messages containing neither `1234` nor any other decimal derived from the value's length, and a static assertion that no `errors.py` constructor parameter is named `length`, `size`, or `count`
- [ ] T026 [P] Unit tests `tests/unit/test_prompt.py`: stdin reader strips exactly one `\n` and one `\r\n`, keeps interior newlines, rejects empty and newline-only input with exit 7; `confirm` defaults No on empty line; `confirm_exact` accepts display or lowercase namespace. Note in the module docstring that no-echo behavior (FR-003) is provided by `getpass` and is verified manually in quickstart scenario 1, not by this file
...
- [ ] T032 [P] [US1] Integration test `tests/integration/test_end_to_end.py::test_set_and_stdin` (create file): run the CLI through `cli_env` with a stdin value into the temp keychain, then read back with `real_store.get` and compare bytes; assert process stdout and stderr contain no value; call `assert_sandbox_clean` so no file under `HOME`, `TMPDIR`, or cwd was created or contains the value (FR-036, SC-009)

### Implementation for User Story 1

- [ ] T033 [US1] Implement `src/keychain_cli/commands/set_.py`: `run_set(args, store, prompts) -> int` following research R-006 and R-007: validate namespace then all variable names (reject duplicates within the invocation), decide stdin vs interactive mode, collect values, `store.add` with `AlreadyExists` handling (confirm / `--force` / refuse), build `StoreReport`, return 0 if nothing skipped else 4. Wrap the per-variable loop so that `KeyboardInterrupt` prints the partial `StoreReport` (what was stored before the interrupt) to stderr and returns `EXIT_INTERRUPTED` (130); add this case to T030's tests
...
- [ ] T037 [P] [US2] Integration tests in `tests/integration/test_end_to_end.py`: `run ns -- /usr/bin/env` output contains the variables; parent test process environment unchanged before and after; `run ns -- sh -c 'exit 42'` returns 42; `run ns -- no-such-cmd-xyz` returns 8; a child that sleeps is sent SIGINT via the tool's PID (which after `exec` **is** the child's PID) and the test polls `os.kill(pid, 0)` for up to 5 s until it raises `ProcessLookupError`, with no `pgrep`; every case calls `assert_sandbox_clean`. Note in the docstring that stdio inheritance (FR-026) is inherent to `exec` and verified manually in quickstart scenario 3
...
- [ ] T044 [P] [US3] Integration test in `tests/integration/test_end_to_end.py`: import quickstart scenario 4's file into the temp keychain, then `run ns -- sh -c 'printf ...'` reproduces `one|override|single # not a comment|three`; `assert_sandbox_clean` with the `.env` file registered as test-written
...
- [ ] T050 [P] [US4] Integration test in `tests/integration/test_end_to_end.py`: store into `Client-A` and `other`, `list --all` prints `Client-A` then `other`; `list client-a` prints sorted variables; stdout contains no value; `assert_sandbox_clean`
...
- [ ] T054 [P] [US5] Integration test in `tests/integration/test_end_to_end.py`: three variables, delete one, two remain; `delete ns --force` removes all and `list --all` no longer shows it; `assert_sandbox_clean`
...
- [ ] T058 [P] [US6] Unit tests `tests/unit/test_cmd_init_check.py`: `init` prompts only for missing variables in manifest order, existing untouched; all present → `namespace <name> is complete`, exit 0, zero prompts; non-interactive with missing → exit 7 listing names; malformed manifest → exit 7 and store unchanged; `KeyboardInterrupt` after the first stored value → partial report names it, exit 130; `check` prints missing names (and `--json`), exit **10** when missing, exit 0 when complete; manifest missing → exit 3 for both
...
- [ ] T060 [P] [US6] Integration test in `tests/integration/test_end_to_end.py`: quickstart scenario 7 end to end in the sandbox `work/` directory (manifest registered as test-written); `check` returns 10 then 0; `assert_sandbox_clean`
...
- [ ] T062 [US6] Implement `src/keychain_cli/commands/init.py`: `run_init(args, store, prompts) -> int` and `src/keychain_cli/commands/check.py`: `run_check(args, store) -> int` per contract (`check` returns `EXIT_CHECK_MISSING` = 10 when any variable is missing, 0 otherwise); `init` reuses the prompting, add, and interrupt-handling logic from `set_.py` (extract a shared `store_values(...)` helper into `set_.py` if needed rather than duplicating)
...
- [ ] T070 [P] [US7] Integration tests `tests/integration/test_clipboard.py` (marker `integration`, additionally skipped when a pasteboard server is unavailable — detect by running `pbpaste` once): `copy` then `pbpaste` equals the placeholder; run `_clipclear` directly with interval `1` and matching hash, then poll `pbpaste` every 0.25 s for up to 5 s until empty (no fixed sleep); run with interval `1` after `pbcopy` of other text, poll the same way and assert the other text is still present after the helper exits (wait on its PID); restore the prior clipboard content in teardown
...
- [ ] T077 [P] Help-text audit: for every subcommand assert in `tests/unit/test_help.py` that `--help` output includes one example invocation, the exit codes it can return, and (for `copy`) the clipboard caveat and the "prefer `run`" sentence; assert top-level `--help` lists all commands with one-line summaries; **FR-004 parser check**: walk the argparse subparser tree and assert every positional is a namespace, variable name, file path, or command tail, and that no option's name contains `value`, `secret`, `password`, or `token`
- [ ] T078 [P] Exit-code contract test `tests/unit/test_exit_codes.py`: parametrize every enumerated error condition in `contracts/cli.md` (codes 1–10 and 130) and assert the documented code; assert `errors.py` codes match the README table (parse the table)
- [ ] T079 [P] Performance check `tests/integration/test_perf.py`: `run ns -- /usr/bin/true` median wall time over 5 runs under 1 s locally (SC-007) and under 3 s when the `CI` environment variable is set, to absorb hosted-runner variance without becoming flaky; assert lazy imports by checking `sys.modules` after `run` excludes `keychain_cli.envfile`, `keychain_cli.manifest`, `keychain_cli.clipboard`
